...
# 사이즈가 다른 이미지/ 흑백 컬러 이미지 resize, 배열하는 함수
...
def getContours(img,imgContour):
    ...
    for cnt in contours:               # cnt : 검출된 윤곽선들이 저장된 Numpy 배열
        ...
        if area > areaMin:
            # 검출된 윤곽선 그리기(이미지, 윤곽선, -1:모든 윤곽선, 색,두께)
            ...
            # 근사치가 적용된 윤곽선의 똑바로 선 경계 사각형 그리기((x,y): 좌상단 좌표, w:폭 h:높이)
            x , y , w, h = cv2.boundingRect(approx)
            cx = int(x + (w / 2))  # 도형 무게중심 x좌표
            cy = int(y + (h / 2))  # 도형 무게중심 y좌표
